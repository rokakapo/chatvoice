    ...
    companion object {
        ...
        fun startRecording(outputPath: String, context: Context) {
            try {
                ...
                mediaRecorder?.apply {
                    // Try VOICE_CALL first, fallback to VOICE_COMMUNICATION, then MIC
                    try {
                        setAudioSource(MediaRecorder.AudioSource.VOICE_CALL)
                    } catch (e: Exception) {
                        Log.w(TAG, "VOICE_CALL source unavailable, trying VOICE_COMMUNICATION")
                        try {
                            setAudioSource(MediaRecorder.AudioSource.VOICE_COMMUNICATION)
                        } catch (e2: Exception) {
                            Log.w(TAG, "VOICE_COMMUNICATION source unavailable, falling back to MIC")
